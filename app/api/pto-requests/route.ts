...
export async function POST(request: NextRequest) {
  try {
    ...
    const { employeeId, employeeName, startDate, endDate, totalDays, year } = body;

    if (!employeeId || !employeeName || !startDate || !endDate || totalDays === undefined || !year) {
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { id, employeeId, employeeName, startDate, endDate, totalDays, year } = body;
